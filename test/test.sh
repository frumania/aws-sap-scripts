...
echo "Boot Instance"
instance_id=$(aws ec2 run-instances --launch-template LaunchTemplateId=lt-0224bf3ab77c362a2,Version=5 --query 'Instances[].[InstanceId]' --output text)
echo $instance_id
echo "Wait 4 min"
sleep 300
...
echo "Wait 4 min"
sleep 300
result=$(aws ssm list-command-invocations --instance-id $instance_id --command-id "$sh_command_id" --details --query "CommandInvocations[].CommandPlugins[].Status")
echo $result
