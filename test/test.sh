...
echo "Boot Instance"
instance_id=$(aws ec2 run-instances --launch-template LaunchTemplateId=lt-0224bf3ab77c362a2,Version=7 --query 'Instances[].[InstanceId]' --output text)
echo $instance_id
echo "Wait 4 min"
sleep 240
...
echo "Wait 3 min"
sleep 180
result=$(aws ssm get-command-invocation --instance-id $instance_id --command-id "$sh_command_id" --query "Command.Status")
echo $result
